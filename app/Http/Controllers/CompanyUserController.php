...
//        $jobs['id_language'] = $request['id_language'];
    ...
    public function getDeleteJob($id){
        $jobs = Job::find($id);
        $recruitment = Recruitment::where('job_id',$id)->delete();
        $saveJob = SaveTheJob::where('job_id',$id)->delete();
        $jobs->delete();
    ...
    public function getListUser(){
        $company    = Auth::guard('company_user')->user();
        $zones      = Zones::all();
        $jobs       = Job::where('company_user_id', $company->id)->paginate(5);
        $users      = User::all();
...
public function checkJob($id){
    ...
    $checkon = Job::find($id)->update(['status'=> 1]);
    return back();
}
public function unCheckJob($id){
    $company = Auth::guard('company_user')->user();

    $checkon = Job::find($id)->update(['status'=> 0]);
    return back();
}
public function searchUser(Request $request){
    $kw=$request->searchuser;
    $language = User::where('language','like', '%'.$kw.'%')->orderBy('id','DESC')->paginate(9);

    // if ( $language[0]->id == ) {
    //     $user == User::all()->where('id_language',$language[0]->id);
    // }

    return view('pages.employer.showsearch',['language'=>$language]);
}
